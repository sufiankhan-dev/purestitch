...
//       doc.text(
//         `${index + 1}. ${item.name} - Quantity: ${item.quantity} - Price: PKR ${Math.round(item.price)}`,
//         20,
//         yPos
//       );
//       yPos += 10;
...
//             <tfoot>
//               <tr>
//                 <td colSpan={2} className="text-right font-semibold py-4">
//                   Total:
//                 </td>
...
export default function OrderSummaryPage({
  ...
  const generatePDF = () => {
    ...
    orderSummary.cartItems.forEach((item, index) => {
      doc.text(
        `${index + 1}. ${item.name} - Size: ${item.selectedSize} - Quantity: ${item.quantity} - Price: PKR ${Math.round(item.price)}`,
        20,
        yPos
      );
      yPos += 10;
